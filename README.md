...
This repo includes `render.yaml` for a **new Render deployment from scratch** using Neon PostgreSQL via connection string.

### 0) Security first (important)

If you shared your Neon connection string publicly, rotate the Neon database password first, then use the new URL in Render.
...
4. In the Render service, set environment variables before first deploy:
...
Use the Render public URL in your resume portfolio/projects section.

### Notes

- `APP_KEY` is auto-generated by the blueprint.
- `DB_SSLMODE=require` is set for Neon TLS.
- For credential changes later, update `DB_URL` in Render and redeploy.
